...
struct raop_ctx_s *raop_create(uint32_t host, char *name,
	...
	ctx->thread = xTaskCreateStaticPinnedToCore( (TaskFunction_t) rtsp_thread, "RTSP", RTSP_STACK_SIZE, ctx, 
												 ESP_TASK_PRIO_MIN + 2, ctx->xStack, ctx->xTaskBuffer, 1 /* OE: was CONFIG_PTHREAD_TASK_CORE_DEFAULT (=-1 in IDF 5.4); static-pinned variant asserts core ID < 2 */);
#endif
...
static bool handle_rtsp(raop_ctx_t *ctx, int sock)
{
	...
	if (!strcmp(method, "OPTIONS")) {
		...
		ctx->active_remote.thread = xTaskCreateStaticPinnedToCore( (TaskFunction_t) search_remote, "search_remote", SEARCH_STACK_SIZE, ctx, 
																	...
																	1 /* OE: was CONFIG_PTHREAD_TASK_CORE_DEFAULT (=-1 in IDF 5.4); static-pinned variant asserts core ID < 2 */ );
#endif		
		...
		// OE 2026-05-27: if iOS sends a fresh ANNOUNCE+SETUP on the same
		// RTSP TCP socket (rename / network blip / app foreground
		// dance), the previous rtp_t lives on — its receive thread
		// keeps reading from its UDP sockets and firing the data_cb,
		// racing the new rtp_t's decoder into audio_io_write_pcm's 44k
		// resampler statics. Both rtp_t instances also alias the SAME
		// PSRAM jitter buffer because airplay.c's RAOP_SETUP handler
		// gates the alloc on `if (!s_rtp_buffer)`. End the old one
		// before allocating new ports / threads.
		if (ctx->rtp) {
			LOG_INFO("[%p]: SETUP while old rtp_t alive — tearing it down first", ctx);
			rtp_end(ctx->rtp);
			ctx->rtp = NULL;
		}
...
static char *rsa_apply(unsigned char *input, int inlen, int *outlen, int mode)
{
	...
	*/
	...
	// Ported 2026-05-27 to mbedTLS 3.x API:
	//   - mbedtls_pk_parse_key gained f_rng/p_rng args (for blinding).
	//   - mbedtls_rsa_context.len became private; use mbedtls_rsa_get_len.
	//   - MBEDTLS_RSA_PRIVATE / mode arg removed; encrypt is public-only,
	//     so the AUTH branch (PKCS#1 v1.5 private encrypt) is now done
	//     via mbedtls_rsa_pkcs1_sign with MD_NONE — that's equivalent to
	//     "raw PKCS#1 v1.5 private-key transform" the old code wanted.
	//   - decrypt no longer takes a mode arg; padding mode is set on ctx.
	extern int esp_random_wrap(void *p, unsigned char *buf, size_t len);
	mbedtls_pk_init(&pkctx);
	mbedtls_pk_parse_key(&pkctx, (const unsigned char *)super_secret_key,
		sizeof(super_secret_key), NULL, 0,
		esp_random_wrap, NULL);
	...
	case RSA_MODE_AUTH:
		...
		mbedtls_rsa_pkcs1_sign(trsa, esp_random_wrap, NULL, MBEDTLS_MD_NONE,
			inlen, input, outbuf);
		*outlen = rsa_len;
		break;
	case RSA_MODE_KEY:
		...
		outbuf = malloc(rsa_len);
		mbedtls_rsa_pkcs1_decrypt(trsa, esp_random_wrap, NULL,
			&olen, input, outbuf, rsa_len);
		*outlen = olen;
		break;
	}
